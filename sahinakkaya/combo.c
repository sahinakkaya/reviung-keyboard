...
const uint16_t PROGMEM c_caps[] = {SHFT_CAPS, SHFT_BSLS, COMBO_END};
const uint16_t PROGMEM c_copy[] = {ALT_Z, KC_V, COMBO_END};
...
combo_t key_combos[] = {
  ...
  [CUT] = COMBO(c_cut, C(KC_X)),
  [COPY] = COMBO(c_copy, C(KC_C)),
  [PASTE] = COMBO(c_paste, C(KC_V)),
